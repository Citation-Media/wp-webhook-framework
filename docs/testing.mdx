...
## Action Scheduler mapping

`.wp-env.json` maps `vendor/woocommerce/action-scheduler` to `woocommerce/action-scheduler` at the WordPress root. In a real install the framework sits in the consumer's vendor tree and resolves Action Scheduler at `../../../woocommerce/action-scheduler`; here it is mounted at `wp-content/wpwf-framework`, so that same relative path lands at the WordPress root. The mapping recreates the production layout so the suite exercises the real resolution path instead of a test-only fallback.

:::caution
`wp-env` only creates a mapping when its source path already exists at start, and skips it silently otherwise. On a fresh checkout `vendor/` does not exist until `composer:install` has run — and that runs inside the container. `test:integration` therefore starts `wp-env` a second time after installing dependencies, so the mapping is created. Removing that second start makes the whole suite fail with `Class "ActionScheduler" not found`.
:::
